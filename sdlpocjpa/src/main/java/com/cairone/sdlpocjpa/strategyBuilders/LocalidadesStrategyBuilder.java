...
import com.cairone.sdlpocjpa.entities.QLocalidadEntity;
...
public class LocalidadesStrategyBuilder {
    ...
    private void buildFromSelectByKey(SelectByKeyOperation selectByKeyOperation) {
        ...
        String codigoPostal = keys.get("codigoPostal").toString();
        
        BooleanExpression exp = qLocalidad.pais.id.eq(paisId).and(qLocalidad.provincia.id.eq(provinciaId)).and(qLocalidad.locPostal.eq(codigoPostal));
    	...
    	if(criteria instanceof ComparisonCriteria) {
            ...
            if(comparisonCriteria.getLeft() instanceof PropertyCriteriaValue && comparisonCriteria.getRight() instanceof LiteralCriteriaValue) {
                ...
                case "PAISID":
                {
                	...
                	BooleanExpression exp = qLocalidad.pais.id.eq(paisIdValue);
                	this.expression = this.expression == null ? exp : this.expression.and(exp);
                	break;
                }
                case "PROVINCIAID":
                {
                	...
                	BooleanExpression exp = qLocalidad.provincia.id.eq(idValue);
                	this.expression = this.expression == null ? exp : this.expression.and(exp);
                	break;
                }
                case "CODIGOPOSTAL":
                {
                	...
                	BooleanExpression exp = qLocalidad.locPostal.eq(descripcionValue);
                    this.expression = this.expression == null ? exp : this.expression.and(exp);
                	break;
                }
                case "DESCRIPCION":
                {
                	String descripcionValue = (String) value;
                	BooleanExpression exp = qLocalidad.descripcion.eq(descripcionValue);
                    this.expression = this.expression == null ? exp : this.expression.and(exp);
                	break;
                }
                case "PREFIJO":
                {
                	Integer idValue = Integer.valueOf(value.toString());
                	BooleanExpression exp = qLocalidad.prefijo.eq(idValue);
                	this.expression = this.expression == null ? exp : this.expression.and(exp);
                	break;
                }
                }
